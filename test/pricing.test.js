...
import { estimateCostForEvents, estimateCostForGroups, estimateEventCost, LONG_CONTEXT_INPUT_THRESHOLD } from "../src/pricing.js";
...
test("unknown prices and incomplete details stay outside the cost estimate", () => {
  const estimate = estimateCostForEvents([
    {
      model: "gpt-6-sol",
      detailMask: 3,
  ...
  assert.equal(estimate.totalUsd, null);
  assert.equal(estimate.cacheHitRate, 20 / 80);
  assert.equal(estimate.modelCount, 2);
  assert.equal(estimate.pricedTokens, 0);
  assert.equal(estimate.unpricedTokens, 100);
  assert.deepEqual(estimate.unpricedModels, ["custom-review-model", "Unknown model"]);
});
...
test("missing cache-write detail leaves input unpriced while retaining known output cost", () => {
  ...
  assert.equal(estimate.inputUsd, 0);
  assert.equal(estimate.cachedInputUsd, 0);
  assert.equal(estimate.cacheWriteInputUsd, 0);
  assert.equal(estimate.outputUsd, 0.0001);
  assert.equal(estimate.totalUsd, 0.0001);
  assert.equal(estimate.pricedTokens, 10);
  assert.equal(estimate.unpricedTokens, 100);
  assert.equal(estimate.cacheWriteUnknownTokens, 100);
});
